            ...
            if (user_id) {
                ...
                if (getFavorites.length === 0) {
                    return res
                        ...
                        .json({
                            message:
                                'First add favorite movies to get recommendations',
                        })
                }

                for (const favorite of getFavorites) {
                    const recommendedMovies =
                        await MovieService.getRecommendations(
                            favorite.id,
                            lang,
                            page,
                        )
                ...
                const recommendedMovies = await MovieService.getRecommendations(
                    movie_id,
                    lang,
                    page,
                )
                if (recommendedMovies && recommendedMovies.results) {
                    recommendations = recommendedMovies.results.filter(
                ...
                return res
                    ...
                    .json({
                        message:
                            'Invalid request. Provide either user_id or movie_id',
                    })
            }
